...
const codeGenerationSchema = {
    ...
    properties: {
        html: {
            type: Type.STRING,
            description: "The HTML code for the component. It should be a single string, containing only the body's content, without <html> or <body> tags."
        },
        css: {
            type: Type.STRING,
            description: "The CSS code for styling the component. It should be a single string, without <style> tags."
        },
        javascript: {
            type: Type.STRING,
            description: "The JavaScript code for the component's functionality. It should be a single string, without <script> tags."
        }
    },
    required: ["html", "css", "javascript"]
};

export const formatCode = async (code: GeneratedCode): Promise<GeneratedCode | null> => {
    try {
        const fullPrompt = `
        ...
        const formattedCode: GeneratedCode = JSON.parse(jsonString);
        return formattedCode;

    } catch (error) {
        console.error("Error formatting code with Gemini API:", error);
        throw new Error("Failed to communicate with the AI for formatting. Please try again.");
    }
};

export const generateCode = async (prompt: string): Promise<GeneratedCode | null> => {
    try {
        const fullPrompt = `
        ...
        const generatedCode: GeneratedCode = JSON.parse(jsonString);
